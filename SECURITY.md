...
- `config.yaml` is git-ignored and never committed. It's where your ntfy
  ...
  `username`/`password`) live — keep it local to the host running the
  service.
...
- If you accidentally commit a secret, treat it as compromised: rotate the
  ntfy token/password immediately, then remove it from git history (e.g. with
  `git filter-repo` or by force-pushing a rewritten history), not just delete
  it in a new commit.

## Network exposure

- The service only makes outbound HTTPS requests to `api.open-meteo.com`,
  `air-quality-api.open-meteo.com`, and your configured ntfy server. It does
